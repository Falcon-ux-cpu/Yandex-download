...
def clear_yandex_trash():
    """Новая функция: полностью очищает корзину Яндекс.Диска"""
    headers = {"Authorization": f"OAuth {CONFIG['yandex_token']}"}
    try:
        res = requests.delete("https://cloud-api.yandex.net/v1/disk/trash/resources", headers=headers)
        if res.status_code in [202, 204]:
            print("Корзина Яндекс.Диска успешно очищена.")
        else:
            print(f"Не удалось очистить корзину: {res.status_code} {res.text}")
    ...
    try:
        with MailBox(CONFIG["imap_server"]).login(CONFIG["email_user"], CONFIG["email_pass"]) as mailbox:
            ...
            for msg in messages:
                ...
                # Очистка после обработки каждого конкретного письма
                clear_yandex_trash()
                
    except Exception as e:
        print(f"Ошибка выполнения: {e}")
    
    # Финальная очистка корзины в самом конце работы сессии
    clear_yandex_trash()
